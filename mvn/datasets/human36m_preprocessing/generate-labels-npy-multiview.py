...
) # shape = (7,4) / 각 Subject 마다, 각 Camera마다의 Parameter 값을 가지고 있는 행렬 : retval['cameras']

# Create a data type object (Subject Index, Action Index, Frame Index, Key Points, bbox_by_camera_tlbr*)
...
h36m_root = sys.argv[1] # $THIS_REPOSITORY/data/human36m

destination_file_path = os.path.join(h36m_root, "extra", f"human36m-multiview-labels-{BBOXES_SOURCE}bboxes.npy")

una_dinosauria_root = sys.argv[2] # $THIS_REPOSITORY/data/human36m/extra/una-dinosauria-data/h36m
cameras_params = h5py.File(os.path.join(una_dinosauria_root, 'cameras.h5'), 'r')

# Fill retval['cameras']
for subject_idx, subject in enumerate(retval['subject_names']): # For each Subject (subject_idx=0, subject=S1)
    for camera_idx, camera in enumerate(retval['camera_names']): # For each Camera (camera_idx=0, camera=54138969)
        assert len(cameras_params[subject.replace('S', 'subject')]) == 4
        # una_dinosauria 의 cameras.h5 파일이 이런식으로 구성되어 있나보다!
        camera_params = cameras_params[subject.replace('S', 'subject')]['camera%d' % (camera_idx+1)] # cameras_params[subject1][camera1]
        camera_retval = retval['cameras'][subject_idx][camera_idx] # Shape (7,4) 중 하나를 가져온다.
        
        def camera_array_to_name(array):
            return ''.join(chr(int(x[0])) for x in array)

        assert camera_array_to_name(camera_params['Name']) == camera # 54138969 가 이름이 맞는지 확인하는 과정인듯
        """
...
# Fill bounding boxes
bboxes = np.load(sys.argv[3], allow_pickle=True).item() # $THIS_REPOSITORY/data/human36m/extra/bboxes-Human36M-GT.npy
...
for subject_idx, subject in enumerate(retval['subject_names']): # subject=S1
    subject_path = os.path.join(h36m_root, "processed", subject) # data/human36m/processed/S1 ..
    ...
    for action_idx, action in enumerate(retval['action_names']): # action=Phoning-1 (Actions that corresponding subject did)
        action_path = os.path.join(subject_path, action, 'imageSequence') # data/human36m/processed/S1/Phoning-1/imageSequence
        if not os.path.isdir(action_path):
            raise FileNotFoundError(action_path)

        for camera_idx, camera in enumerate(retval['camera_names']): # camera=54138969
            camera_path = os.path.join(action_path, camera) # data/human36m/processed/S1/Phoning-1/imageSequence/54138969
            if os.path.isdir(camera_path):
                frame_idxs = sorted([int(name[4:-4])-1 for name in os.listdir(camera_path)]) # Sort by frames
        ...
        # THIS_REPOSITORY/data/human36m/extra/una-dinosauria-data/h36m/S1/MyPoses/3D_positions/[Action Name]
        ...
        table_segment['keypoints'] = poses_world # True label of the images
